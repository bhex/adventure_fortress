...
fn sync_actors(
    ...
    let alive: Vec<(String, Role)> = game
        .0
        .inhabitants
        .get_alive()
        .iter()
        .map(|i| (i.name.clone(), i.role))
        .collect();

    for (entity, actor) in actors.iter() {
        if !alive.iter().any(|(n, _)| n == &actor.name) {
            commands.entity(entity).despawn();
        }
    }

    let mut rng = rand::rng();
    for (name, role) in alive {
...
fn assign_beds(actors: &[(String, Role)], layout: &MapLayout) -> HashMap<String, IVec2> {
    let mut barracks: Vec<IVec2> = if layout.built.contains(&Upgrade::Barracks) {
    ...
    let mut sorted: Vec<&(String, Role)> = actors.iter().collect();
    sorted.sort_by(|a, b| a.0.cmp(&b.0));

    let mut beds = HashMap::new();
    // guards claim barracks bunks first
    for (name, _) in sorted.iter().filter(|(_, r)| *r == Role::Guard) {
...
fn preferred_anchors(role: Role) -> &'static [AnchorKind] {
    match role {
        Role::Guard => &[AnchorKind::Gate, AnchorKind::Walls, AnchorKind::Courtyard],
        Role::Farmer => &[AnchorKind::Farm, AnchorKind::Gate, AnchorKind::Courtyard],
        Role::Blacksmith => &[
...
fn wander(
    ...
    let beds = if bedtime {
        let roster: Vec<(String, Role)> =
            actors.iter().map(|(a, _, _)| (a.name.clone(), a.role)).collect();
    ...
    for (actor, mut pos, mut wander) in actors.iter_mut() {
        ...
        let target = match wander.target {
            ...
            _ => {
                ...
                let mut picked = None;
                for kind in preferred_anchors(actor.role) {
