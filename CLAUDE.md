...
- **Balance / survival** (Stage 5): the harvest scales with field hands (`2*farmers + peasants/2`) so a growing hold feeds itself; Granary food caps (60/100/160/220, +60 DeepCellars) let a summer surplus bridge winter. ~16/20 auto-runs survive to day 150 (`sim band`); the rest mostly lose the *darkness war* (darkness pinned at 100). Regression guard: `an_auto_run_survives_well_past_the_turning_seasons` (content test, seed 4 to day 120).
- **Wild foraging** (`game_state::apply_daily_effects`): when the hold lacks the building for a staple, its laborers (`Role::Peasant` + `Role::Miner`, and `Farmer`s for food) gather it from the wild — capped well below a built yield (wood ≤4, stone ≤3 with **no ore**, food ≤8) so a Lumberyard/Mine/Farm is always the upgrade, and once built its yield replaces the forage. Yield is darkened by danger via the region band (`forage_pct`: Quiet 100% → Gathering 75% → Deep 40% → Overwhelming 15%), so the encroaching dark chokes off the wild economy. No workers, no forage. This is what makes the opening days (and a deliberately building-light hold) survivable.
...
- **Construction — the build queue** (`fortress.rs`, `game_state.rs`): `queue_build()` appends a `BuildProject` (`worker_days_remaining`, `materials_owed`, `funded`) to `Fortress.projects`. The queue is **strict FIFO**: only the front order draws labor, and only once **funded**. `GameState::try_fund_front` pays an order's materials in full the moment it sits at the front and the hold can afford them (so a build queued affordable into an empty queue funds — and pays — at once; anything queued unaffordable or behind others waits its turn). Nothing behind the front ever jumps the line. The daily tick funds the front then `advance_projects(workforce)` draws the front down by `build_workforce()` (laborers — peasants/miners — plus a baseline) and calls `build_upgrade` on completion. `Fortress::move_project(idx, up)` reorders; `GameState::cancel_build(idx)` strikes an order (refunding a *funded* order's materials, nothing for an unfunded one) — both exposed in the build menu's queue panel (▲/▼/✕). Event-granted buildings (`Effect::AddUpgrade`) and the founding charter still build instantly via `build_upgrade`. `BuildAvailability::InProgress` gates re-queuing the same kind; `CantAfford` no longer blocks queuing (you queue on credit) — the menu shows ETA for the funded front and "waiting · costs …" for the rest.
